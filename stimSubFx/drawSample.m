...
switch distrib
    ...
    case 'unif16'
        % discrete uniform distribution, 2-16, 8 points
        dist = 2:2:16;
        dist(2,:) = ones(size(dist));
        
    case 'logspace_1.75_32' % log-spaced values scaling by 1.75, max 32
        m = 32; % maximum delay
        fac = 1.75;
        n = 8;
        d1 = log(m/(fac^n - 1));
        d2 = log(m + m/(fac^n - 1));
        dist = exp(linspace(d1,d2,n+1));
        dist = dist(2:end) - dist(1); % subtract 1st to normalize relative to zero.
        dist(2,:) = ones(size(dist));
