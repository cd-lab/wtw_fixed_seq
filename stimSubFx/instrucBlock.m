function [] = instrucBlock(params,rects,display,blockMins,nBks,bkIdx)
...
switch bkIdx
    case 1
        ...
        msg{2} = sprintf('Each token is worth %d %s%s at first. After some time, the token will "mature" and be worth %d %ss.',...
            params.payoffLo,params.unit,lowValPlural,params.payoffHi,params.unit);
        msg{3} = 'Now try a practice round. Wait until the token matures, then press the spacebar to sell it.';
        txt = sprintf('%s\n\n',msg{:}); txt((end-1):end) = [];
        showMsg(wid,bkgd,txt,respKey);

        % demo trial 1 -- practice waiting
        showTrials(params,rects,display,trialData);

        % instruc screen 2
        Screen('TextSize',wid,rects.txtsize_msg);
        msg = {};
        msg{1} = 'Good. Let''s do it again. Wait until the token matures, then sell it.';
        txt = sprintf('%s\n\n',msg{:}); txt((end-1):end) = [];
        showMsg(wid,bkgd,txt,respKey);
    ...
    case 2
        ...
        Screen('TextSize',wid,rects.txtsize_msg);
        msg = {};
        msg{1} = 'In the next block, you will be doing the same thing as before. The timing and structure of the task may change. The token will mature after some amount of time.';
